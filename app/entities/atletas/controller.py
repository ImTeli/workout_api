...
from app.entities.atletas.schema import AtletaIn, AtletaOut, AtletaUpdate
from app.entities.atletas.model import AtletaModel
...
async def post(
    ...
    categoria = (await db_session.execute(
        select(Categoria).filter_by(nome=categoria_nome))
    ).scalars().first()
    ...
    centro_treinamento = (await db_session.execute(
        select(CentroTreinamento).filter_by(nome=centro_treinamento_nome))
    ).scalars().first()
...
async def query(db_session:db) -> list[AtletaOut]:
...
async def get(id: UUID4, db_session:db) -> AtletaOut:
    atleta: AtletaOut = (
...
async def patch(id: UUID4, db_session:db, atleta_up: AtletaUpdate = Body(...)) -> AtletaOut:
...
async def delete(id: UUID4, db_session:db) -> None:
    atleta: AtletaOut = (await db_session.execute(select(AtletaModel).filter_by(id=id))).scalars().first() # type: ignore
